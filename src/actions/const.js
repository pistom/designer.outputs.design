export const GET_PROJECT_DATA = 'GET_PROJECT_DATA';
export const GET_PROJECT_DATA_FULFILLED = 'GET_PROJECT_DATA_FULFILLED';
export const GET_PROJECT_DATA_PENDING = 'GET_PROJECT_DATA_PENDING';
export const GET_PROJECT_DATA_REJECTED = 'GET_PROJECT_DATA_REJECTED';
export const GET_MESSAGES = 'GET_MESSAGES';
export const GET_MESSAGES_FULFILLED = 'GET_MESSAGES_FULFILLED';
export const GET_MESSAGES_PENDING = 'GET_MESSAGES_PENDING';
export const GET_MESSAGES_REJECTED = 'GET_MESSAGES_REJECTED';
export const SELECT_STEP = 'SELECT_STEP';
export const SET_PROJECT_NAME = 'SET_PROJECT_NAME';
export const SET_PROJECT_PASSWORD = 'SET_PROJECT_PASSWORD';
export const SAVE_PROJECT_DATA = 'SAVE_PROJECT_DATA';
export const SET_NUMBER_OF_VERSIONS = 'SET_NUMBER_OF_VERSIONS';
export const STORE_BACKGROUNDS = 'STORE_BACKGROUNDS';
export const STORE_DEVICES = 'STORE_DEVICES';
export const SET_BREAKPOINT_WIDTH = 'SET_BREAKPOINT_WIDTH';
export const SET_VARIANT_FILENAME = 'SET_VARIANT_FILENAME';
export const SET_VARIANT_IMAGE_SIZE = 'SET_VARIANT_IMAGE_SIZE';
export const SET_VARIANT_DENSITY = 'SET_VARIANT_DENSITY';
export const EDIT_PAGE_NAME = 'EDIT_PAGE_NAME';
export const ADD_NEW_PAGE = 'ADD_NEW_PAGE';
export const GET_FILES_LIST = 'GET_FILES_LIST';
export const GET_FILES_LIST_FULFILLED = 'GET_FILES_LIST_FULFILLED';
export const GET_FILES_LIST_PENDING = 'GET_FILES_LIST_PENDING';
export const GET_FILES_LIST_REJECTED = 'GET_FILES_LIST_REJECTED';
